...
function setupChatRoom() {
    ...
    const pushTavernBtn = document.getElementById('push-tavern-btn');
    if (pushTavernBtn) {
        pushTavernBtn.addEventListener('click', async () => {
            ...
            showPanel('none');
            if (typeof window.showAutoPushModal === 'function') {
                window.showAutoPushModal(binding);
            } else {
                try {
                    const r = await window.TavernSync.pushToTavern(binding);
                    if (r.pushed > 0) showToast(`已推送 ${r.pushed} 条新消息到酒馆`);
                    else showToast('没有新消息需要推送');
                } catch (e) {
...
function openChatRoom(chatId, type) {
    ...
    // 酒馆自动拉取（后台静默，不阻塞聊天打开）
    if (type === 'private' && window.TavernSync) {
        window.TavernSync.autoPullIfNeeded(chatId).catch(() => {});
    }

    // 根据是否绑定酒馆决定是否显示手动推送按钮
    const pushTavernBtn = document.getElementById('push-tavern-btn');
    if (pushTavernBtn) {
        let hasBinding = false;
        if (type === 'private' && window.TavernSync) {
            try { hasBinding = !!window.TavernSync.findBindingForChar(chatId); } catch {}
        }
        pushTavernBtn.style.display = hasBinding ? '' : 'none';
    }
...
async function sendMessage() {
    ...
    // 剧情时间模式：继承最近一条带 storyTime 的消息的时间戳
    if (currentChatType === 'private' && chat.timeMode === 'story' && Array.isArray(chat.history)) {
        for (let i = chat.history.length - 1; i >= 0; i--) {
            if (chat.history[i] && chat.history[i].storyTime) {
                message.storyTime = chat.history[i].storyTime;
                break;
            }
        }
    }
