...
// 格式化剧情时间分割线（输入 "YYYY-MM-DD HH:MM"）
function formatStoryTimeDivider(storyTimeStr) {
    if (!storyTimeStr || typeof storyTimeStr !== 'string') return '';
    const iso = storyTimeStr.trim().replace(' ', 'T');
...
function filterHistoryForAI(chat, historySlice, ignoreContextDisabled = false) {
    ...
    // 1.5 隐藏楼层级联：酒馆里最新被隐藏的楼层 storyTime 之前（含）的 OVO 消息也跟着藏
    // 那些剧情已经被酒馆侧总结到世界书里了，UwU 再发一遍只会让 AI 困惑
    // 改成按切片：找到第一条 storyTime > 切点的消息，保留它及之后的所有消息，之前一律丢掉
    const cutoff = chat.tavernMemory && chat.tavernMemory.hiddenCutoffStoryTime;
    if (cutoff) {
        const cutoffMs = new Date(String(cutoff).replace(' ', 'T')).getTime();
        if (!isNaN(cutoffMs)) {
            let firstKeepIdx = filteredHistory.length; // 没找到任何 > cutoff 的消息 → 全砍
            for (let i = 0; i < filteredHistory.length; i++) {
                const m = filteredHistory[i];
                if (!m.storyTime) continue;
                const ms = new Date(String(m.storyTime).replace(' ', 'T')).getTime();
                if (!isNaN(ms) && ms > cutoffMs) { firstKeepIdx = i; break; }
            }
            filteredHistory = filteredHistory.slice(firstKeepIdx);
        }
    }
